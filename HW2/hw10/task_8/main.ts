...
form.addEventListener('click', function(event: MouseEvent): void {
    ...
    let numRows: number = +(document.querySelector<HTMLInputElement>('#rows')?.value ?? '') || 0;
    //дістаю значення к-ті ячейок
    let numCells: number = +(document.querySelector<HTMLInputElement>('#cells')?.value ?? '') || 0;
    //дістаю контент
    let text: string = document.querySelector<HTMLInputElement>('#contents')?.value ?? '';
